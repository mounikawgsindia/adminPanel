...
class FlashSaleApprovalFragment : Fragment() {

    lateinit var _binding:FragmentFlashSaleApprovalBinding
    val binding get()=_binding
    lateinit var flashSaleAdapter: FlashSaleApprovedAdapter
    var flashSaleList=ArrayList<ApprovedFlashSale>()
    ...
    override fun onCreateView(
        ...
        _binding=FragmentFlashSaleApprovalBinding.inflate(layoutInflater)
        return binding.root
    }
    ...
    private fun setUI(){
        binding.apply {
            ...
            flashsaleApprovedRv.addOnScrollListener(object : RecyclerView.OnScrollListener() {
                override fun onScrolled(recyclerView: RecyclerView, dx: Int, dy: Int) {
                    // Enable swipe-to-refresh only if the RecyclerView is scrolled to the top
                    val isAtTop = !flashsaleApprovedRv.canScrollVertically(-1)
    ...
    private fun setObserver(){
        ...
        viewModel.flashSaleAprovedError.observe(viewLifecycleOwner){error->
            binding.swipeRefreshLayout.isRefreshing=false
            Extensions.showCustomSnackbar(requireContext(),error, R.color.light_red)
        }
        viewModel.flashSaleAprovedResponse.observe(viewLifecycleOwner){response->
            binding.swipeRefreshLayout.isRefreshing=false
            flashSaleList.clear()
            Extensions.showCustomSnackbar(requireContext(),"Success", R.color.green)
            Log.d("observer","observer main class --->$response")
            flashSaleList= response as ArrayList<ApprovedFlashSale>
            if(flashSaleList.size==0){
                binding.flashsaleApprovedRv.visibility = View.GONE
                binding.listEmpty.visibility = View.VISIBLE
                binding.listEmpty.text="Empty List"
            }
            else{
                binding.flashsaleApprovedRv.visibility = View.VISIBLE
    ...
    private fun fetchDataFromNetwork(isRefresh:Boolean){
        ...
        if(Extensions.isNetworkAvailable(requireActivity())){
            viewModel.approvedFlashSaleApi(isRefresh)
        }else{
            Extensions.showNetworkAlertDialog(requireContext())
        }
    }
    private fun setRecycleView(){
        binding.flashsaleApprovedRv.apply {
            flashSaleAdapter= FlashSaleApprovedAdapter(viewModel,requireContext(),flashSaleList)
